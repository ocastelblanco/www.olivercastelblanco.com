...
**Estado 2026-09-21:** CSP ampliada y verificada en vivo (`curl -sI` contra
`ocastelblanco.com` y `www.ocastelblanco.com`, ambos `200`, ambos con la CSP nueva).
Implementación completa y verificada en local: `npm run build`/`lint` en verde, cero
handlers inline en el `dist/`, `gtag` ausente del HTML prerenderizado, banner probado con
`claude-in-chrome` en `localhost:4200` (aparece, ACEPTAR lo oculta, persiste tras recargar,
`// cookies` del topbar lo reabre, cero errores de consola). PR abierto — pendiente merge
del usuario y verificación de `page_view` real en GA4 Tiempo real contra producción.
