...
PR #41 fusionado y desplegado en `production`. El usuario confirmó en el panel real de
Google Tag Manager / Analytics que la visita quedó registrada — evidencia más fuerte que
cualquier verificación de red desde el navegador (GA4 envía sus hits vía
`navigator.sendBeacon`, que las herramientas de inspección de red no siempre capturan).

Verificación adicional tras el merge: `curl -sI` contra `ocastelblanco.com` y
`www.ocastelblanco.com` confirma la CSP nueva en ambos hosts, ambos `200`. Navegación real
con `claude-in-chrome` por `/`, `/proyectos`, `/lab`, `/contacto` en producción: cero
errores de consola, `gtag.js` carga con `200`, banner de consentimiento funcional (acepta,
oculta, persiste, se reabre desde `// cookies` del topbar). Implementación y verificación
local completa ya documentadas en la sesión anterior (ver `MEMORY.md` ADR-015).

**Motor JIT recalculado:** Anti-spam reCAPTCHA v3 (antes Tarea 2) pasa a Tarea 1. Fetch SSR
de The Lab (ADR-011, desplazada desde el 2026-09-20) reingresa como Tarea 2 — sigue siendo
el candidato de mayor prioridad del backlog sin dependencias bloqueantes.
...
muerto. ADR-015 y ADR-016 escritos (renumeradas desde el ADR-014/015 original al
detectarse que ADR-014 ya existía para la entrega SES del formulario de contacto), motor
JIT recalculado. Detalle en `MEMORY.md`
§"Sesión 2026-09-20".

**Tareas desplazadas** (no completadas, siguen vigentes y documentadas en `MEMORY.md` §2):

- *Fetch SSR de The Lab* (gap de SEO, ADR-011) — era Tarea 1. Reingresó al motor JIT el
  2026-09-21 como Tarea 2, al completarse la analítica (ver historial más arriba).
- *`angular.json` copia `lab.dev.json` a todos los builds* (ADR-012 §"paso 8") — era Tarea 2,
  sigue en el backlog, vuelve al motor JIT cuando se libere un slot.
