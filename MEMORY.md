...
- [x] Revisar en Search Console el efecto del 301 de `olivercastelblanco.com` sobre el indexado existente. **Cerrado 2026-09-22** (propiedad de dominio `sc-domain:ocastelblanco.com`, cubre `www` y http/https automáticamente): 8 de 10 páginas conocidas sin indexar, 3 motivos — (1) "Página con redirección" (4, esperado: 2 son `www.ocastelblanco.com` recién arreglado, 2 son rutas legacy del sitio anterior `/inicio` y `/portafolio` que ya no existen); (2) "Duplicada sin canónica" (1, `https://www.ocastelblanco.com/`, último rastreo 11/9/26 — **anterior** al fix del 22/9, se solicitó revalidación manual desde Search Console, estado "iniciada" 22/9/26); (3) "Rastreada, sin indexar" (3, decisión propia de Google — incluye un PDF de currículum `/assets/docs/Resume_OliverCastelblanco.pdf` que **ya no existe en el código actual** — remanente del sitio anterior, caerá solo — y otra ruta legacy `/contacteme`, hoy `/contacto`). `sitemap.xml` verificado correcto: las 7 rutas actuales, sin `www`, sin URLs legacy. Ninguna acción de código requerida — solo la revalidación manual ya solicitada ← era **Tarea 2**
...
- [x] ~~Evaluar un `404` limpio para `/content/*`~~ — **descartado 2026-09-22** (ver §7 Gotchas): el fix correcto requiere Lambda@Edge en `origin-response` (CloudFront no invoca `viewer-response` cuando el origen devuelve ≥400, confirmado con documentación oficial de AWS), infraestructura más pesada de lo que valía el impacto real (bajo, sin datos sensibles expuestos). Además se corrigió la premisa: hoy el objeto faltante devuelve el HTML del **sitio actual** (post-Switch), no del sitio anterior como decía este documento
...
- **Implementado 2026-08-05 — Headers de seguridad (OWASP A05, `CLAUDE.md` §6):**
  ...
  reCAPTCHA v3, ver ADR-015/ADR-016 y la sección "CSP de producción" más abajo para el
  valor vigente):
  ```
...
| `CustomErrorResponses` a nivel de distribución (403/404 → `/index.html`) se resuelven con su **propio** lookup de behavior para el `ResponsePagePath` | Si el `ResponsePagePath` no matchea el `PathPattern` del behavior original de la request, CloudFront lo sirve desde el behavior que sí matchea — **antes de El Switch (2026-08-04) era el behavior por defecto del bucket del sitio anterior; después de El Switch, el mismo lookup resuelve al Lambda SSR de producción** (verificado con `curl` real el 2026-09-22: un objeto faltante en `/content/*` hoy devuelve el HTML del sitio **actual**, no del anterior como se documentó originalmente — la causa del `200` sigue vigente, pero el contenido servido cambió con el switch sin que nadie lo revisara). Un objeto faltante en un behavior nuevo puede terminar devolviendo `200` con contenido de OTRO origen en vez de un `404` limpio. Revisar `CustomErrorResponses` de la distribución antes de asumir que "no existe el objeto" se traduce en un error visible. |
| ⚠️ *(evaluado y descartado 2026-09-22, ver `TODO.md` historial)* CloudFront **no invoca triggers de `viewer-response`** (ni CloudFront Functions ni Lambda@Edge) cuando el origen devuelve un status ≥400, aunque `CustomErrorResponses` lo reescriba después a `200` | Confirmado con documentación oficial de AWS (`aws-mcp search_documentation`, página "Restrictions on all edge functions"). Consecuencia práctica: no se puede "arreglar" un `CustomErrorResponses` que enmascara un `404` con una CloudFront Function liviana en `viewer-response` — el status del **origen** (no el reescrito) determina si el trigger corre. La única forma que sí se dispara siempre es Lambda@Edge en `origin-response` (patrón documentado oficialmente por AWS para exactamente este caso: "Customize 403 error pages from Amazon CloudFront Origin with Lambda@Edge"), que implica desplegar una función nueva en `us-east-1` con replicación global — infraestructura bastante más pesada que una CloudFront Function. Antes de asumir que una CF Function puede interceptar un error de origen, confirmar el código de status que ve el trigger, no solo el que termina viendo el visitante. |
...
### ADR-015 — Analítica web: GA4 directo (no Tag Manager) + Consent Mode v2 sin scripts inline

- **Fecha:** 2026-09-20
- **Estado:** **Implementado y verificado en producción real** (2026-09-21, PR #41). El
  usuario confirmó en el panel de Google Analytics que la visita quedó registrada.
...
- **Consecuencias:**
  - Requiere ampliar la CSP de CloudFront (ver ADR-016, se aplica una sola vez para ambos).
...
- **Decisión:**
  1. **Google reCAPTCHA v3** (invisible, basado en score). Se evaluó **Cloudflare
     ...
     el banner de consentimiento de ADR-015 — pero el usuario eligió reCAPTCHA
...
4. **El honeypot de `/contact` es código muerto** desde que se escribió: el servidor lo
   verifica, el formulario nunca lo expone. Ver ADR-016.
...
nuevas, ADR-015 y ADR-016 escritos. Implementación planificada para el **2026-09-21**.
...
Sesión de planeación, sin código. PR #39 (ADR-015/ADR-016) fusionado y verificado con
...
4. **Título correcto en `page_view`** — ver la corrección del punto 4 de ADR-015.
...
## Sesión 2026-09-21 (2) — Implementación de ADR-015, verificación en producción y cierre de Tarea 1

**Implementación (antes de este cierre):** CSP de CloudFront ampliada en vivo
(`f768cc69-…`, ETag nuevo `E3UN6WX5RRO2AG`, respaldo de la CSP anterior en el scratchpad
de la sesión) antes de escribir código. `AnalyticsService`, banner de consentimiento e
i18n implementados; verificado en local con `claude-in-chrome` (banner funcional, cero
errores de consola) y `npm run build`/`lint` en verde. PR #41 abierto.

**Verificación en producción tras el merge del usuario:** el usuario confirmó
independientemente en el panel de Google Analytics que la visita quedó registrada — la
prueba más fuerte posible, viene del propio servidor de Google. Verificación adicional
desde este lado: `curl -sI` contra `ocastelblanco.com` y `www.ocastelblanco.com` confirma
la CSP nueva en ambos hosts (`200`); navegación real con `claude-in-chrome` por `/`,
`/proyectos`, `/lab`, `/contacto` en producción, cero errores de consola, `gtag.js` carga
con `200` en las 4 rutas. **Limitación de la verificación de red:** GA4 envía sus hits vía
`navigator.sendBeacon`, que la herramienta de inspección de red usada no captura como
petición HTTP visible — no es una falla de la implementación, es una limitación conocida
de captura de beacons; la confirmación del usuario desde el panel real de GA4 cubre ese
punto ciego.

**Hallazgo de esta sesión, corregido de inmediato:** al ir a cerrar ADR-014 en `MEMORY.md`
se descubrió que **ya existía un ADR-014 legítimo** ("Entrega del formulario de contacto
vía Amazon SES", 2026-08-04, línea 716) — la sesión del 2026-09-20 creó "ADR-014" y
"ADR-015" para analítica/anti-spam sin verificar los números ya usados en el documento,
generando una colisión. **Corregido:** renumerados a **ADR-015** (analítica) y **ADR-016**
(anti-spam) en `MEMORY.md`, `TODO.md` y el plan local (`~/.claude/plans/transient-cuddling-moth.md`),
verificado que no queda ningún encabezado `### ADR-` duplicado en el documento
(`grep -n "^### ADR-" MEMORY.md`). **Lección para sesiones futuras:** antes de asignar el
siguiente número de ADR, correr `grep -n "^### ADR-" MEMORY.md` — no asumir que el último
visto en una sección es el máximo real del documento completo.

**Motor JIT recalculado:**
- Tarea 1 (Analítica web, ADR-015) → completada, movida al historial de `TODO.md`.
- Anti-spam reCAPTCHA v3 (ADR-016) → promovida de Tarea 2 a **Tarea 1** (prerrequisitos ya
  cumplidos desde el 2026-09-21, sin dependencias pendientes).
- Fetch SSR de The Lab (ADR-011) → reingresa como **Tarea 2**, restaurada del backlog tras
  haber sido desplazada dos veces desde el 2026-08-05.
